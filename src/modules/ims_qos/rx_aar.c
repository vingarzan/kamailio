...
void async_aar_reg_callback(int is_timeout, void *param, AAAMessage *aaa, long elapsed_msecs) {
    ...
    rx_avp_process_3gpp_user_location_information(aaa, &access_network_information);
    rx_avp_process_3gpp_access_network_charging_identifier(aaa,&access_network_charging_info);
    ...
    if (cdp_result >= 2000 && cdp_result < 3000) {
        ...
        if (is_rereg) {
            ...
            result = CSCF_RETURN_TRUE;
            create_complex_return_code(result, visited_network_id, access_network_information, access_network_charging_info);
            goto done;
        }
        //need to set Rx auth data to say this session has been successfully opened
        //This is used elsewhere to prevent acting on termination events when the session has not been opened
...
error:
    //set failure response code
    create_complex_return_code(result, visited_network_id, access_network_informationm, access_network_charging_info);


done:
    if (access_network_charging_info.s) pkg_free(access_network_charging_info.s);
    if (visited_network_id.s) pkg_free(visited_network_id.s);    
    if (access_network_information.s) pkg_free(access_network_information.s);
    if (t) tmb.unref_cell(t);
    //free memory
    if (aaa)
        cdpb.AAAFreeMessage(&aaa);
