...
int rx_mcc_mnc_to_sip_visited(str* dst, str src)
{
	...
	mcc = (100*(src.s[0] - '0') + (10 * (src.s[1] - '0') ) + ((src.s[2]-'0')));
	if (mcc<0 || mcc>=999) {
	...
	if (src.len == 5) {
		...
		mnc = ((src.s[5] - '0') + (10 * (src.s[4] - '0') ) + (100*(src.s[3]-'0')));
	} else {
	...
	dst.len = snprintf(dst.s, VISITED_ID_MAX_LENGTH,"ims.mnc%03d.mcc%03d.3gppnetwork.org",mnc, mcc);
	return 1;
}
...
int rx_avp_process_3gpp_sgsn_mcc_mnc(AAAMessage *aaa, str *dst) {
	...
	for (avp = aaa->avpList.head;avp;avp=avp->next) {
		if (avp->code == AVP_EPC_3GPP_SGSN_MCC_MNC && (avp->flags & AAA_AVP_FLAG_VENDOR_SPECIFIC) && avp->vendorId == IMS_vendor_id_3GPP) {
			break;
		}
	}
	if (!avp) return 0;
...
int rx_avp_extract_mcc_mnc(str src, int *mcc, int *mnc) {
	int c =0;
	if (src.len < 3 || !src.s || !mcc || !mnc) return 0;
	c = ;
	*mcc = (src.s[0] & 0x0F)*100 + ((src.s[0] & 0xF0)>>4)*10 + (src.s[1] & 0x0F);	
	if (((src.s[1] & 0xF0)>>4)==0x0F) {
		//ignore MNC digit 3 means its 0 at the front
		*mnc = (src.s[2] & 0x0F)*10 + ((src.s[2] & 0xF0)>>4);
	} else {
		*mnc = (src.s[2] & 0x0F)*100 + ((src.s[2] & 0xF0)>>4)*10 + ((src.s[1] & 0xF0)>>4)
	}
	return 1;
...
{	
	...
	if (!cdp_avp.epcapp.get_3GPP_User_Location_Info(rar->avpList, &data, 0)) {
		...
		length = data.len - 1; // that's the payload length independent of what it says
		p=data.s+1;
		...
		if (tai.len && ecgi.len) {
			...
			tac |= (tai.s[3]<<8);
			eci = ((ecgi.s[0] & 0x0F)<<24) + (ecgi.s[1]<<16) + (ecgi.s[2]<<8)+ ecgi.s[3];
			...
			if (tac & 0xFF00) {
				dst->len = snprintf(dst->s, dst->len,"%3u%u%08"PRIx16"%08"PRIx32"",mcc,mnc,tac,eci);
			} else {
				dst->len = snprintf(dst->s, dst->len,"%3u%u%08"PRIx8"%08"PRIx32"",mcc,mnc,tac,eci);
			}
			if (dst->len>1) return 1;
		}		
 ...
 */
int rx_add_supported_features(AAA_AVP_List* msglist, uint32_t vendorid, uint32_t feature_list_id, uint32_t feature_list) {
	AAA_AVP_List list = {0};
	if (!msglist) return 0;
...
int rx_avp_process_3gpp_access_network_charging_identifier(AAAMessage *msg, str *dst) {
	...
	for (avp = msg->avpList.head;avp;avp=avp->next) {
		if (avp->code == AVP_IMS_Access_Network_Charging_Identifier) break;
	}	
	if (avp) {
		//Ungroup the AVP?
